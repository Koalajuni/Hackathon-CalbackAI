...
class SliderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (mode == CircularSliderMode.doubleHandler) {
      ...
      canvas.drawCircle(initHandler, 16.0, handler); 
      // canvas.drawShadow(, handler, 16.0, true);   //Todo --->  size of Start handler
      canvas.drawCircle(initHandler, handlerOutterRadius, handlerOutter);
    }

    endHandler = radiansToCoordinates(center, -pi / 2 + endAngle, radius);
    canvas.drawCircle(endHandler, 16.0, handler); //Todo --->  size of End handler
